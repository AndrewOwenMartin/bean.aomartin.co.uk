...
The happiness of the miners can be measured probabilistically, or represented with an absolute boolean value, so long as each miner is either happy or unhappy by the end of each day [82].
...
Though ‘happy’ is a term that is similarly subjective as one’s dining preferences, in this case it is used in an objective sense.
All miners share an identical process whereby the amount of gold they locate on a single day defines a probability that the miner will declare themselves ‘happy’ at the end of the day when miners congregate to potentially share the identity of the hills they are mining.
